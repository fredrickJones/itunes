...
app.controller('mainCtrl', function($scope, itunesService, $localStorage){
  ...
  var movieColDefs = [
        {field: 'Play', displayName: 'Play', width: '40px', cellTemplate: '<div class="ngCellText" ng-class="col.colIndex()"><a href="{{row.getProperty(col.field)}}"><img src="http://www.icty.org/x/image/Miscellaneous/play_icon30x30.png"></a></div>'},
        {field: 'Artist', displayName: 'Director'},
        {field: 'Collection', displayName: 'Album'},
        {field: 'SongName', displayName: 'Song'},
        {field: 'AlbumArt', displayName: 'Album Art', width: '110px', cellTemplate: '<div class="ngCellText" ng-class="col.colIndex()"><img src="{{row.getProperty(col.field)}}"></div>'},
        {field: 'Type', displayName: 'Type'},
  ...
  //First inject itunesService into your controller.
    ...
    if($localStorage.songData) {
      $scope.songData = $localStorage.songData
    };
  ...
  $scope.getSongData = function() {
    itunesService.getArtist($scope.artist).then(function(res) {
      $scope.songData = res;
      $localStorage.songData = $scope.songData;
    });
  };
